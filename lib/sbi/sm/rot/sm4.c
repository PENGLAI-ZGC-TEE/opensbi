...
#include <stdint.h>
#include "rot/sm4_reg.h"
#include "rot/top_earlgrey.h"

#include <sm/print.h>
#define LOG_INFO printm_err
...
int run_rot_sm4(void) {
  ...
  LOG_INFO("\nINPUT KEY_ENC IS:");
  LOG_INFO("%08x%08x%08x%08x\n",*SM4_KEY_3_REG_ADDR,*SM4_KEY_2_REG_ADDR,*SM4_KEY_1_REG_ADDR,*SM4_KEY_0_REG_ADDR);
  ...
  //LOG_INFO(strings);
  LOG_INFO("%08x%08x%08x%08x\n",*SM4_RESULT_OUT_3_REG_ADDR,*SM4_RESULT_OUT_2_REG_ADDR,*SM4_RESULT_OUT_1_REG_ADDR,*SM4_RESULT_OUT_0_REG_ADDR);
  SM4_OFF();
...
//接下来是解密过程
  ...
  LOG_INFO("### %08x%08x%08x%08x ###\n",out_data[3],out_data[2],out_data[1],out_data[0]);//将数组中的值显示
  SM4_OFF();
